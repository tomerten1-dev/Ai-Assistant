...
(async () => {
  try {
    setAge(1);
    const fresh = await handleChat({ messages: [{ role: 'user', content: 'זוג בלי ילדים, פברואר, בולגריה' }], slots: {} });
    setAge(40);
    const old = await handleChat({ messages: [{ role: 'user', content: 'זוג בלי ילדים, פברואר, בולגריה' }], slots: {} });
