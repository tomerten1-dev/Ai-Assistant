...
(async () => {
  ...
  await t('Belambra = all-inclusive club, Club du Soleil = full board + wine + equipment — on the card, said once, and again under "יקר לי"', async () => {
    ...
    // L'Oree des Pistes is half board — not like the rest (Tomer, 13/09)
    const oree = r[0].cards.find(c => /Oree/.test(c.hotel));
    if (oree) assert.ok(!oree.highlight_he, 'the half-board Belambra was tagged all-inclusive');
    ...
    assert.ok(/קלאב דו סוליי כולל במחיר פנסיון מלא \(שלוש ארוחות ביום\), יין בארוחות, ציוד סקי\/סנובורד\./.test(s[0].reply_he), s[0].reply_he.slice(0, 160));
    assert.ok(!/מסביר את המחיר/.test(s[0].reply_he + r[0].reply_he), 'Tomer\'s wording only');
