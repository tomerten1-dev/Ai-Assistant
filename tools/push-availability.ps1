<#
  ...
  שלוש דרכים להריץ את זה, לפי כמה גישה יש לך:

  א. ידנית מהמחשב שלך — לא נוגע בשום מערכת של אף אחד.
     F: ממופה אצלך ממילא, אז:   npm run push
     זה מספיק לגמרי לפיילוט. מריצים בבוקר, ובסוף היום אם נמכר הרבה.

  ב. משימה מתוזמנת על המחשב שלך — אותו דבר, בלי לזכור.
     Task Scheduler על התחנה שלך. עדיין אפס גישה למערכות אחרות.
     החיסרון היחיד: כשהמחשב שלך כבוי, המלאי לא מתעדכן (והבוט יודע
     להתנהג בהתאם — ראו INVENTORY_STALE_HOURS).

  ג. על המחשב שמארח את F: — הפתרון הקבוע, וזה מה שדורש אישור IT.
     רק אחרי שהפיילוט הוכיח את עצמו. ההוראות המלאות למטה.
